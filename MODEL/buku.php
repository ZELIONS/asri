<?php
require_once 'C:/xampp/htdocs/backend/model/config.php';
class Buku
{
    private $id;
    private $judul;
    ...
    // Constructor
    public function __construct($id, $judul, $penulis, $penerbit, $sinopsis, $tahun_terbit, $gambar, $bahasa, $jumlah_halaman)
    {
        $this->id = $id;
        $this->judul = $judul;
    ...
    function TambahBuku()
    {
        $sql = "INSERT INTO buku (judul, penulis, penerbit, sinopsis, tahun_terbit,gambar, bahasa, jumlah_halaman) VALUES (?, ?, ?, ?, ?,?,?,?)";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("ssssissi", $this->judul, $this->penulis, $this->penerbit, $this->sinopsis, $this->tahun_terbit, $this->gambar, $this->bahasa, $this->jumlah_halaman);
        $result = $stmt->execute();
        $stmt->close();
        return $result;
    }
    ...
    function EditBuku()
    {
        $sql = "UPDATE buku SET judul=?, penulis=?, penerbit=?, sinopsis=?, tahun_terbit=?, bahasa=?, jumlah_halaman=?, gambar=? WHERE id=?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("ssssisssi", $this->judul, $this->penulis, $this->penerbit, $this->sinopsis, $this->tahun_terbit, $this->bahasa, $this->jumlah_halaman, $this->gambar, $this->id);
        $result = $stmt->execute();
        $stmt->close();
    ...
    {
        $sql = "SELECT buku.judul,buku.gambar, user.username AS username_peminjam,  peminjaman.tanggal_peminjaman, peminjaman.tanggal_pengembalian, peminjaman.id
        FROM peminjaman
...
$bukuModel = new Buku(null, null, null, null, null, null, null, null, null);
